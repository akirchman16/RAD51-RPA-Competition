...
RAD51_Timer = zeros(1,N);   %set of timers to record dwell time of RAD51 proteins
RPA_A_Timer = zeros(1,N);   %set of timers to record dwell time of RPA-A proteins
...
RAD51_DwellTime = [];   %total time on the lattice for each RAD51 monomer
RPA_A_DwellTime = [];   %total time on lattice for each RPA-A protein
RPA_D_DwellTime = [];   %total time on lattice for each RPA-D protein
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    RAD51_Timer(RAD51_Mon_BoundAtSpot ~= 0) = RAD51_Timer(RAD51_Mon_BoundAtSpot ~= 0) + dt(Event_Count);  %advances a timer wherever a RAD51 monomer is bound
    RPA_A_Timer(RPA_A_BoundAtSpot ~= 0) = RPA_A_Timer(RPA_A_BoundAtSpot ~= 0) + dt(Event_Count);  %advances timer wherever RPA-A is bound to lattice
    RPA_D_Timer(RPA_D_BoundAtSpot ~= 0) = RPA_D_Timer(RPA_D_BoundAtSpot ~= 0) + dt(Event_Count);  %advances timer wherever RPA-D is bound to lattice
    ...
    elseif sum(Full_Propensity(1:10)) > Randoms(2)*a_0 %RAD51 Monomer Unbinding Occurs
        ...
        RAD51_DwellTime = [RAD51_DwellTime, RAD51_Timer(RAD51_M_Unbind_Spot) + dt(Event_Count)];  %records final amount of dwell time for the protein
        RAD51_Timer(RAD51_M_Unbind_Spot) = 0;   %resets the timer for the monomer
    elseif sum(Full_Propensity(1:11)) > Randoms(2)*a_0 %RAD51 Dimer Unbinding Occurs
        ...
        RAD51_DwellTime = [RAD51_DwellTime, RAD51_Timer(RAD51_D_Unbind_Spot)+dt(Event_Count)];   %finalizes total time on the lattice for one monomer
        RAD51_DwellTime = [RAD51_DwellTime, RAD51_Timer(RAD51_D_Unbind_Spot+n_RAD51)+dt(Event_Count)];  %total time in which the second monomer was bound to lattice
        RAD51_Timer([RAD51_D_Unbind_Spot,RAD51_D_Unbind_Spot+n_RAD51]) = 0; %resets timer where monomers wer bound
    elseif sum(Full_Propensity(1:12)) > Randoms(2)*a_0 %RPA Macro Dissociation
        ...
        if DNA(1,RPA_Macro_Unbind_Spot+n_A) == RPA_D  %if the D part of RPA is hinged open...
            ...
            RPA_D_DwellTime = [RPA_D_DwellTime,RPA_D_Timer(RPA_Macro_Unbind_Spot+n_A)+dt(Event_Count)];   %finalizes timer for RPA-D protein
            RPA_D_Timer(RPA_Macro_Unbind_Spot+n_A) = 0; %resets timer for this location
        end
        ...
        RPA_A_DwellTime = [RPA_A_DwellTime,RPA_A_Timer(RPA_Macro_Unbind_Spot)+dt(Event_Count)]; %finalizes timer for RPA_A
        RPA_A_Timer(RPA_Macro_Unbind_Spot) = 0; %resets timer for RPA-A at corresponding location
    elseif sum(Full_Propensity(1:13)) > Randoms(2)*a_0 %RPA-D Micro Dissociation
        ...
        RPA_D_DwellTime = [RPA_D_DwellTime,RPA_D_Timer(RPA_D_Micro_Unbind_Spot) + dt(Event_Count)]; %advances timer for RPA-D component (finalizes it)
        RPA_D_Timer(RPA_D_Micro_Unbind_Spot) = 0;   %resets timer for this location
