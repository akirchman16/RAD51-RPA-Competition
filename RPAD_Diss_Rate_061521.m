...
RPA_D_Diss_Timer(1) = 0;    %initial time since RPA-D Dissociation is 0
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
RPA_D_Velocity = [];    %initializes an empty array to record RPA-D Dissociation velocities
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    RPA_D_Diss_Timer = RPA_D_Diss_Timer+dt(Event_Count);    %advances timer since last RPA-D Dissociation event
    ...
    elseif sum(Full_Propensity(1:13)) > Randoms(2)*a_0 %RPA-D Micro Dissociation
        ...
        RPA_D_Velocity = [RPA_D_Velocity,1/RPA_D_Diss_Timer];  %records velocity (events/time interval)
        RPA_D_Diss_Timer = 0;   %resets RPA-D dissociation timer
...
% figure(2);
% subplot(1,2,1); %velocity of RPA-D dissociation over time
% scatter(t(LocationHistory(13,:) ~= 0),RPA_D_Velocity,3,'blue','filled');
% hold on;
% xlabel('Time');
% xlim([0 MaxTime]);
% ylabel('Velocity (Events/Time Interval)');
% ylim([0 inf]);
% title('RPA-D Dissociation Velocity');
% box on;
% subplot(1,2,2); %histogram of RPA-D Dissociation Velocities
% histogram(RPA_D_Velocity,round(sqrt(numel(RPA_D_Velocity))));
% xlabel('Velocity (Events/Time Interval)');
% xlim([0 inf]);
% ylabel('Count');
% ylim([0 inf]);
