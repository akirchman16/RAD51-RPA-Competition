...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    parfor a = 1:length(RAD51_M_Available_I_Gap_Edges(1,:))
    ...
    parfor b = 1:length(RAD51_D_Available_I_Gap_Edges(1,:))
    ...
    parfor c = 1:length(RPA_Available_I_Gap_Edges(1,:))
    ...
    parfor d = find(RPA_D_HingedOpen == 1)   %check each location where RPA-D is hinged open
    ...
    if numel(RAD51_D_Filament_Locations) > 0   %if there are any filaments containing dimers...
        parfor e = 1:numel(RAD51_D_Filament_Locations)     %...check each one to add it to the list of dimer locations
            if RAD51_D_Filament_Lengths(e) == 2*n_RAD51    %if there's only one dimer in the filament...
                ...
                parfor f = 1:Monomers_per_Dimer_Filament(e)-1     %record that there's a possible dimer at each monomer location (except the last one)
